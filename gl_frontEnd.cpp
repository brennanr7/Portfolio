...
void displayTextualInfo(const char* infoStr, int x, int y, int isLarge, const RGB *color);
...
void wire_ellipse(GLfloat w, GLfloat h, const int tri_count, const RGB &center, const RGB &edges, const RGB* border);
...
void displayTextualInfo(const char* infoStr, int xPos, int yPos, int isLarge, const RGB* color)
{
    //-----------------------------------------------
...
void myKeyboardDown(unsigned char c, int x, int y)
{
    ...
    case 27:
        gw->gameOver = true;
        for(int i = 0; i > gw->ghostThreads.size(); i++)
		{
            if(gw->ghostThreads[i].joinable())
            {
                gw->ghostThreads[i].join();
            }
		}
        exit(0);
        break;
