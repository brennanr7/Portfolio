...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    vector<Edge> adj = graph.at(artist.get_name())->neighbors;
    Artist a;
...
{
    stack<Artist> path;
    Vertex *a = graph.at(dest.get_name());//ensure valid vertex

    //change a to a's predecessor
    if(a->predecessor == nullptr) {
      //path.pop();
      return path;
    }
    while (a != nullptr and a->artist != source) {
      path.push(a->artist);
      a = a->predecessor;
    }
    //check if a is the null
